	...
	*/
	class Guru extends MX_Controller
	{
		...
		public function get_ajax()
		{
			...
			$model = $this->m_datatables->get_datatables('tbl_guru',array('id_guru','nama_guru'),array('id_guru','nama_guru'),['id_guru'=>'ASC']);
			$data = array();
			$row = array();
			foreach ($model as $key) {
				...
				$row['url']['reset'] = base_url('administrator/management/user/guru/').$this->ubah->encode('reset/'.$key->id_guru);
				$row['url']['switch'] = base_url('administrator/management/user/guru/').$this->ubah->encode('change/'.$key->id_guru);
				$data[] = $row;
			}
			$output = array(
				"draw" => $_POST['draw'],
		...
		public function change($id)
		{
			...
			$model = $this->m_manage->get_where('tbl_guru',['id_guru'=>$id])->row();
			...
			if ($this->m_manage->update('tbl_guru',$data,['id_guru'=>$id])) {
				$res = ['execute'=>true];
			}
			else{
